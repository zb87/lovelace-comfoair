...
export class ComfoAirCard extends LitElement {
  ...
  render(): TemplateResult {
    ...
    const cardTitle =
      this.config.name ??
      (climateEntity?.attributes?.friendly_name as string) ??
      "ComfoAir";
      ...
      <ha-card>
        ...
        <div class="diagram-wrapper">
          ...
          >
            ...
            <path
              class="house-fill"
              d="M130,68 L275,12 L420,68 V222 H130 Z"
            />
            <path
              class="house-outline"
              d="M130,68 L275,12 L420,68 V222 H130 Z"
            ...
            <!-- Section labels (Vertically aligned at y=68, directly above top temperatures) -->
            <text class="label" x="75" y="68" text-anchor="middle">
              ${localize("common.outdoor", this.hass, this.config)}
            </text>
            <text class="label" x="295" y="68" text-anchor="middle">
            ...
            ${bypass
              ? svg`
                <g class="bypass-badge clickable" @click=${() => this.openMoreInfo("binary_sensor." + this.entityPrefix + "_bypass_open")} title="${localize("common.open_bypass_details", this.hass, this.config)}">
                  <rect class="bypass-badge-bg" x="145" y="138" width="80" height="20" rx="10"/>
                  <text class="bypass-badge-text" x="185" y="152" text-anchor="middle">⇌ ${localize("common.bypass", this.hass, this.config)}</text>
                </g>
              `
              : null}
            ...
            <text class="temp-text clickable" x="75" y="91" text-anchor="middle" fill="${outsideClr}" @click=${() => this.openMoreInfo("outside_temperature")} title="${localize("sensors.outside_temperature", this.hass, this.config)}">${outsideStr}°C</text>
            <text class="temp-text clickable" x="295" y="91" text-anchor="middle" fill="${upperIndoorClr}" @click=${() => this.openMoreInfo(upperIndoorTempSensor)} title="${localize(`sensors.${upperIndoorTempSensor}`, this.hass, this.config)}">${upperIndoorStr}°C</text>
            <text class="temp-text clickable" x="75" y="169" text-anchor="middle" fill="${exhaustClr}" @click=${() => this.openMoreInfo("exhaust_temperature")} title="${localize("sensors.exhaust_temperature", this.hass, this.config)}">${exhaustStr}°C</text>
            <text class="temp-text clickable" x="295" y="169" text-anchor="middle" fill="${lowerIndoorClr}" @click=${() => this.openMoreInfo(lowerIndoorTempSensor)} title="${localize(`sensors.${lowerIndoorTempSensor}`, this.hass, this.config)}">${lowerIndoorStr}°C</text>

            <!-- Fan data (clickable for RPM & air level info) -->
            <text class="fan-text clickable" x="75" y="134" text-anchor="middle" @click=${() => this.openMoreInfo("intake_fan_rpm")} title="${localize("sensors.intake_fan_rpm", this.hass, this.config)}">${intakeRpm} rpm</text>
            <text class="fan-text clickable" x="295" y="134" text-anchor="middle" @click=${() => this.openMoreInfo(upperIndoorLvlSensor)} title="${localize(`sensors.${upperIndoorLvlSensor}`, this.hass, this.config)}">${upperIndoorLvl}%</text>
            <text class="fan-text clickable" x="75" y="212" text-anchor="middle" @click=${() => this.openMoreInfo("exhaust_fan_rpm")} title="${localize("sensors.exhaust_fan_rpm", this.hass, this.config)}">${exhaustRpm} rpm</text>
            <text class="fan-text clickable" x="295" y="212" text-anchor="middle" @click=${() => this.openMoreInfo(lowerIndoorLvlSensor)} title="${localize(`sensors.${lowerIndoorLvlSensor}`, this.hass, this.config)}">${lowerIndoorLvl}%</text>
          </svg>
        </div>
  ...
  private bypassGradients(
    ...
    return svg`
      <linearGradient id="grad-bypass-upper" x1="20" y1="0" x2="350" y2="0" gradientUnits="userSpaceOnUse">
        <stop offset="0%" stop-color="${outsideClr}"/>
        <stop offset="100%" stop-color="${supplyClr}"/>
      </linearGradient>
      <linearGradient id="grad-bypass-lower" x1="20" y1="0" x2="350" y2="0" gradientUnits="userSpaceOnUse">
  ...
  private renderHXDucts(
    ...
    return svg`
      ...
      <path class="duct"
        d="M 20,97 L 130,97 Q 145,97 155,104 L 215,168 Q 225,175 240,175 L 350,175 L 350,199 L 240,199 Q 220,199 205,187 L 145,123 Q 135,121 120,121 L 20,121 Z"
        fill="url(#grad-hx1)"
      />
      <rect class="duct" x="20" y="97" width="10" height="24" rx="2" fill="${outsideClr}"/>
      <rect class="duct" x="340" y="175" width="10" height="24" rx="2" fill="${supplyClr}"/>

      <!-- Duct 2: Stale air (Top-right indoor -> Bottom-left outdoor) -->
      <path class="duct"
        d="M 350,97 L 240,97 Q 225,97 215,104 L 155,168 Q 145,175 130,175 L 20,175 L 20,199 L 130,199 Q 150,199 165,187 L 225,123 Q 235,121 250,121 L 350,121 Z"
        fill="url(#grad-hx2)"
      />
      <rect class="duct" x="340" y="97" width="10" height="24" rx="2" fill="${returnClr}"/>
      <rect class="duct" x="20" y="175" width="10" height="24" rx="2" fill="${exhaustClr}"/>
    `;
  }

  private renderBypassDucts() {
    return svg`
      <rect class="duct" x="20" y="97" width="330" height="24" rx="4" fill="url(#grad-bypass-upper)" />
      <rect class="duct" x="20" y="175" width="330" height="24" rx="4" fill="url(#grad-bypass-lower)" />
    `;
  }
  ...
  private renderFlowArrows(bypass: boolean) {
    if (bypass) {
      return svg`
        ...
        ${this.chevron(50, 109, "right")}
        ${this.chevron(100, 109, "right")}
        ${this.chevron(270, 109, "right")}
        ${this.chevron(320, 109, "right")}

        <!-- Bottom duct: Right to Left -->
        ${this.chevron(320, 187, "left")}
        ${this.chevron(270, 187, "left")}
        ${this.chevron(100, 187, "left")}
        ${this.chevron(50, 187, "left")}
      `;
    }
    ...
    return svg`
      ...
      ${this.chevron(50, 109, "right")}
      ${this.chevron(100, 109, "right")}
      ${this.chevron(270, 187, "right")}
      ${this.chevron(320, 187, "right")}

      <!-- Stale air return: Right to Left -->
      ${this.chevron(320, 109, "left")}
      ${this.chevron(270, 109, "left")}
      ${this.chevron(100, 187, "left")}
      ${this.chevron(50, 187, "left")}
    `;
  }
